...
# This is a modified version of the Rolling Hough Transform
# designed by Susan E. Clark, originally available at 
# https://seclark.github.io/RHT/. 

# This version will return data rather than writing it
# to a .fits file, and integrates the RHT convolution. 

# All modified sections are prefixed and postfixed with MOD.
...
def window_step(data, wlen, frac, smr, original, smr_mask, wlen_mask, 
    ...
    """
    MOD - returns data rather than writes to disk.
    ...
    if not BUFFER:
        # BEGIN MOD
        bp=np.divide(backproj, np.amax(backproj))
        return([
            np.array(Hi), 
            np.array(Hj), 
            np.array(Hthets), 
